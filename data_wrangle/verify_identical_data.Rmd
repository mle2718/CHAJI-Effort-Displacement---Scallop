...
---
# Purpose
This is code to make sure that Greg, Min-Yang, and Marina's data processing produces the same data.
...
minyang_directory<-file.path(net,"home2", "mlee","Effort-Displacement---Scallop")
marina_directory<-file.path(net,"home4", "mchaji","Effort-Displacement---Scallop")

greg_directory<-file.path(net,"homeX", "gardini","Effort-Displacement---Scallop")
...
datasets_list<-list.files(path=file.path(minyang_directory,"data","main"), pattern="final_product_lease_")
...
final_product_lease_lee <- readRDS(file.path(minyang_directory,"data","main",paste0("final_product_lease_",lee_vintage_string,".Rds")))
Scallop_Linkingorg_lee <- readRDS(file.path(minyang_directory,"data","intermediate",paste0("Scallop_Linkingorg_",lee_vintage_string,".Rds")))
RESULT_COMPILED_lee <- readRDS(file.path(minyang_directory,"data","intermediate",paste0("RESULT_COMPILED_",lee_vintage_string,".Rds")))
...
#This code looks into min-yang's and sets the vintage_string according to the most recent data
datasets_list<-list.files(path=file.path(marina_directory,"data","main"), pattern="final_product_lease_")
datasets_list<-gsub("final_product_lease_","",datasets_list )
datasets_list<-gsub(".Rds","",datasets_list)
datasets_list<-gsub(".csv","",datasets_list)
...
final_product_lease_chaji <- readRDS(file.path(marina_directory,"data","main",paste0("final_product_lease_",chaji_vintage_string,".Rds")))

Scallop_Linkingorg_chaji <- readRDS(file.path(marina_directory,"data","intermediate",paste0("Scallop_Linkingorg_",chaji_vintage_string,".Rds")))
RESULT_COMPILED_chaji <- readRDS(file.path(marina_directory,"data","intermediate",paste0("RESULT_COMPILED_",chaji_vintage_string,".Rds")))
...
```{r lookup_greg_data, eval=FALSE,include=TRUE}
#This code looks into min-yang's and sets the vintage_string according to the most recent data
...
```{r readin_ardini_data, eval=FALSE, include=TRUE}
