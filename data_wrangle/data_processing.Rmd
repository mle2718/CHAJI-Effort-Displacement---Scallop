...
# Constructing scallop fishing years

```{r scallop_fishing_years, echo=TRUE}

Scallops$Scallop_fishing_year<-"None"
Scallops$Scallop_fishing_year[Scallops$Date >= "2006-03-01" & Scallops$Date <= "2007-02-28"] <- "FY2007"
Scallops$Scallop_fishing_year[Scallops$Date >= "2007-03-01" & Scallops$Date <= "2008-02-29"] <- "FY2008"
Scallops$Scallop_fishing_year[Scallops$Date >= "2008-03-01" & Scallops$Date <= "2009-02-28"] <- "FY2009"
Scallops$Scallop_fishing_year[Scallops$Date >= "2009-03-01" & Scallops$Date <= "2010-02-28"] <- "FY2010"
Scallops$Scallop_fishing_year[Scallops$Date >= "2010-03-01" & Scallops$Date <= "2011-02-28"] <- "FY2011"
# Lines under dont work for some reason :,(
Scallops$Scallop_fishing_year[Scallops$Date >= "2011-03-01" & Scallops$Date <= "2012-02-27",] <- "FY2012"

Scallops$Scallop_fishing_year[Scallops$Date >= "2012-03-01" & Scallops$Date <= "2013-02-28",] <- "FY2013"
Scallops$Scallop_fishing_year[Scallops$Date >= "2013-03-01" & Scallops$Date <= "2014-02-29",] <- "FY2014"
Scallops$Scallop_fishing_year[Scallops$Date >= "2014-03-01" & Scallops$Date <= "2015-02-29",] <- "FY2015"
Scallops$Scallop_fishing_year[Scallops$Date >= "2015-03-01" & Scallops$Date <= "2016-02-29",] <- "FY2016"

Scallops$Scallop_fishing_year[Scallops$Date >= "2016-03-01" & Scallops$Date <= "2017-03-01",] <- "FY2017"

Scallops$Scallop_fishing_year[Scallops$Date >= "2016-03-01" & Scallops$Date <= "2017-02-29",] <- "FY2018"
...
lease <- st_transform(lease, crs = st_crs(final_product_lease))
# same results as st_within
inter <- sf::st_intersects(final_product_lease, lease)
inter_save <- inter 
 
 if (any(lengths(inter) > 1)) { # if more than one zone intersects, assign to closest zone
  
  dub <- which(lengths(inter) > 1)
  inter[dub] <- st_nearest_feature(final_product_lease[dub,], lease)
 }
# Add ZoneID column to data
pts <- as.data.frame(as.numeric(inter))
colnames(pts) <- "col.id"
final_product_lease$lease <- lease$NAME[pts$col.id]
final_product_lease$lease <- lease$NAME[as.numeric(inter)]
...
# Zone Assignments 
tenMinSqr_new <- st_transform(tenMinSqr_new, crs = st_crs(final_product_lease))

# same results as st_within
inter <- sf::st_intersects(final_product_lease, tenMinSqr_new)
...
inter_save <- inter 
 ...
 if (any(lengths(inter) > 1)) { # if more than one zone intersects, assign to closest zone
  ...
  dub <- which(lengths(inter) > 1)
  inter[dub] <- st_nearest_feature(final_product_lease[dub,], tenMinSqr_new)
 }
...
final_product_lease$ZoneID <- pts$ID
