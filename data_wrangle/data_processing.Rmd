...
```{r shortcut_dmis, include=TRUE, echo=TRUE, eval=FALSE}
##### copy over a frozen DMIS table from a location on the network.

# Pick the most recent APSD_DMIS2 file. 
mlee_data_vintage<-list.files(path=file.path(net,"home2","mlee","Effort-Displacement---Scallop","data","intermediate"), pattern="APSD_DMIS_2_")
...
type<-strsplit(APSD_DMIS_2$dbsource[1],"_")[[1]][1]

if(type=="frozen_GARFO"){
...
VTR_DMIS_AC <- VTR_DMIS_AC %>%
               ...
               mutate(DOLLAR_ALL_SP_2020=DOLLAR_ALL_SP/value,
                      DOLLAR_2020=DOLLAR/value,
                      TRIP_COST_WINSOR_2020_DOL=TRIP_COST_NOMINALDOLS_WINSOR/value,
                      TRIP_COST_2020_DOL=TRIP_COST_NOMINALDOLS/value)
...
Agg_DOL_POUN_LAND <- VTR_DMIS_AC %>%
  group_by(TRIPID) %>%
 summarise(Agg_DOLLAR = sum(DOLLAR), Agg_POUNDS = sum(POUNDS), Agg_LANDED = sum(LANDED), Agg_DOLLAR_2020=sum(DOLLAR_2020))
