...
<!---
You might want to run this code. To do so, set extract_switch=FALSE
You might want to use a LIVE version of DMIS. To do so, set LIVE_DMIS to TRUE
...
DMIS_STYLE <-"live"
#DMIS_STYLE may be "live", "frozen_GARFO", or "none".  Running the code with DMIS_STYLE="frozen_GARFO" takes a very long time to pull data from the servers.
VTR_extract_switch<-TRUE
scallop_linking_extract_switch<-TRUE
...
} else if(DMIS_STYLE=="none"){
# Do nothing
}

if (DMIS_STYLE=="frozen_GARFO" | DMIS_STYLE=="live"){
...
```{r VTR_Query, echo=TRUE, results=FALSE,eval=VTR_extract_switch}
...
```{r DMIS_Query, echo=TRUE,eval=scallop_linking_extract_switch}
...
#Import Data
Cost_Part1 <- read_excel(file.path(cost_directory1,"2000_2009_Commercial_Fishing_Trip_Costs.xlsx"))
Cost_Part2 <- read_excel(file.path(cost_directory2,"2010_2021_Commercial_Fishing_Trip_Costs.xlsx"))

#Merge Part1 of costs with Part 2 of costs 
#all_yrs_costs<-rbind(Cost_Part1,Cost_Part2)
all_yrs_costs <- merge(Cost_Part1,Cost_Part2, all = TRUE)


keeps<-c("VTR_TRIPID","DB_LANDING_YEAR", "OBSERVED_COST_DUMMY", "TRIP_COST_NOMINALDOLS", "TRIP_COST_NOMINALDOLS_WINSOR")

all_yrs_costs<-subset(all_yrs_costs,select=keeps)
#save to RDS
