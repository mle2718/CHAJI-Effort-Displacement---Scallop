...
spacepanels_directory<-file.path(mlee_dir,"dropoff","wind")
...
APSD_DMIS_2 <- tbl(con, sql("select * from APSD.DMIS_WIND_TEST@garfo_nefsc")) %>%
  mutate("year" = lubridate::year(DATE_TRIP)) %>%
  mutate("month" = lubridate::month(DATE_TRIP)) %>%
  mutate("day" = lubridate::day(DATE_TRIP)) %>%
  dplyr::select(-DATE_TRIP) %>% collect()
