...
params:
    ...
    input_shapefile:
      value: "wind_NY2.RDS"
      choices: ["wind_sf_final.RDS","wind_sf_nocable.RDS","wind_NY2.RDS"]
...
```{r}
scallop0322MainDataTable %>% 
  summarize(across(.cols = c("DOLLAR_2022", "KILOGRAMS", "TRIP_LENGTH"), 
...
boxplot(scallop0322MainDataTable$DOLLAR_2022,
  main = "Revenue in 2020 Dollars",
  xlab = "USD$",
  ylab = "Revenue",
  col = "white",
  border = "black",
...
boxplot(scallop0322MainDataTable$KILOGRAMS,
  main = "Catch in Kilograms",
  xlab = "Kilograms",
  ylab = "Catch",
  col = "white",
  border = "black",
...
boxplot(scallop0322MainDataTable$TRIP_LENGTH,
  main = "Trip Length in Days",
  xlab = "Trip Length",
  ylab = "Days",
  col = "white",
  border = "black",
...
<br><br><br>
Observations inside/outside closures by OPERNUM. 
```{r}
...
<br><br><br>
Observations inside/outside closures by month and fleet. 
```{r}
agg_helper(scallop0322MainDataTable, value = "in_closure", 
            group = c("month", "fleet"), 
            count = TRUE) %>% 
...
```{r eval=AA_DAS_param_hack, echo=AA_DAS_param_hack}
zone_out <- 
  scallop0322MainDataTable %>%
  add_count(ZoneID) %>% 
  filter(n >= 10) %>%
  zone_summary(project = "scallop0322",
               ...
               zone.spat = "MN10SQID",
               output = "tab_plot",
               breaks = pretty(.$n, n = 10),
               bin_colors = fishset_viridis(length(pretty(.$n, n = 10))),
               count = TRUE,
               na.rm = TRUE)
...
zone_out <- 
  scallop0322MainDataTable %>% 
  mutate(MEAT_KG_total = sum(MEAT_KG)) %>% 
  dplyr::select(ZoneID, MEAT_KG, MEAT_KG_total) %>% 
  group_by(ZoneID) %>% 
  mutate(MEAT_KG = sum(MEAT_KG)/MEAT_KG_total * 100, n = n()) %>% 
  filter(n >= 10) %>% 
  distinct(ZoneID, MEAT_KG) %>% 
  ungroup() %>% 
  zone_summary(project = project,
               spat = scallop0322TenMNSQRSpatTable, 
               zone.dat = "ZoneID",
...
zone_out$plot
zone_out$table %>% 
  pretty_lab(cols = c("MEAT_KG"), type = "scientific") %>%
  pretty_tab_sb(width = "40%")
```
...
zone_out <- 
  ...
  dplyr::select(ZoneID, MEAT_KG, MEAT_KG_total) %>% 
  group_by(ZoneID) %>% 
  mutate(MEAT_KG = sum(MEAT_KG)/MEAT_KG_total * 100,
         n = n()) %>% 
  filter(n >= 10) %>% 
  distinct(ZoneID, MEAT_KG) %>% 
  ungroup() %>% 
...
zone_out <- 
  scallop0322MainDataTable %>% 
  mutate(TRIP_total = sum(TRIP_LENGTH)) %>% 
  dplyr::select(ZoneID, TRIP_LENGTH, TRIP_total) %>% 
  group_by(ZoneID) %>% 
  mutate(TRIP_LENGTH = sum(TRIP_LENGTH)/TRIP_total * 100, n = n()) %>% 
  filter(n >= 10) %>% 
  distinct(ZoneID, TRIP_LENGTH) %>% 
  ungroup() %>% 
  zone_summary(project = project,
               spat = scallop0322TenMNSQRSpatTable, 
               zone.dat = "ZoneID",
...
zone_out <- 
  ...
  dplyr::select(ZoneID, TRIP_LENGTH, TRIP_total) %>% 
  group_by(ZoneID) %>% 
  mutate(TRIP_LENGTH = sum(TRIP_LENGTH)/TRIP_total * 100,
         n = n()) %>% 
  filter(n >= 10) %>% 
  distinct(ZoneID, TRIP_LENGTH) %>% 
  ungroup() %>% 
...
zone_out <- 
  scallop0322MainDataTable %>% 
  add_count(ZoneID) %>%
  filter(n >= 10 & CPUE_p >= .025 & CPUE_p <= .975) %>%
  zone_summary(project = project,
           spat = scallop0322TenMNSQRSpatTable, 
           zone.dat = "ZoneID",
           zone.spat = "MN10SQID",
...
zone_out <- 
  scallop0322MainDataTable %>% 
  add_count(ZoneID) %>%
  filter(n >= 10 & VPUE_p >= .025 & VPUE_p <= .975) %>% 
  zone_summary(project = project,
           spat = scallop0322TenMNSQRSpatTable, 
           zone.dat = "ZoneID",
           zone.spat = "MN10SQID",
...
zone_out$table %>% 
  arrange(fleet,-OPERATING_PROFIT_2022) %>%
  pretty_lab() %>% 
  pretty_tab_sb(width = "60%")
...
### TRIP_COST_WINSOR_2022_DOL


```{r}
outlier_table(scallop0322MainDataTable, project,
              x = "TRIP_COST_WINSOR_2022_DOL") %>% 
  pretty_lab() %>% 
  pretty_tab()
```
...
```{r}
temp_plot(scallop0322MainDataTable, project,
          var.select = "TRIP_COST_WINSOR_2022_DOL",
          len.fun = "percent",
          agg.fun = "sum",
          date.var = "scallop_fishing_yearD")
```
...
```{r}
temp_plot(scallop0322MainDataTable, project,
          var.select = "OPERATING_PROFIT_2022",
          len.fun = "percent",
          agg.fun = "sum",
          date.var = "scallop_fishing_yearD")
```
...
```{r}
xy_plot(scallop0322MainDataTable, project,
        var1 = "TRIP_LENGTH", var2 = "DOLLAR_2022",
        regress = FALSE)
```
...
```{r}
xy_plot(scallop0322MainDataTable, project,
        var1 = "TRIP_LENGTH", var2 = "TRIP_COST_2022_DOL",
        regress = FALSE)
```
...
```{r}
xy_plot(scallop0322MainDataTable, project,
        var1 = "TRIP_LENGTH", var2 = "TRIP_COST_WINSOR_2022_DOL",
        regress = FALSE)
```
...
catch_out$table %>% 
  pretty_lab(cols = "DOLLAR_ALL_SP_2022") %>%
  pretty_tab()
...
catch_out$table %>% 
  pivot_wider(names_from = GEARCODE, values_from = DOLLAR_ALL_SP_2022) %>% 
  pretty_lab() %>%
  pretty_tab_sb() 
...
catch_out$table %>% 
  pretty_lab(cols = "OPERATING_PROFIT_2022") %>%
  pretty_tab()
