...
`r params$LA_GCIFQ_only`.
```{r retain_LA_GCIFQ, eval=params$LA_GCIFQ_only}

scallop0322MainDataTable <- 
  scallop0322MainDataTable %>% 
  filter(fleet == "Limited Access" | fleet == "GCIFQ")
...
### 1. Aggregate DOLLAR, POUNDS, LANDED variables of the same name,which is totally sketchy.
Agg_DOL_POUN_LAND <- scallop0322MainDataTable %>%
  group_by(TRIPID) %>%
  summarise(DOLLAR = sum(DOLLAR), POUNDS = sum(POUNDS), LANDED = sum(LANDED))
...
#Force drop the DOLLAR, POUNDS, and LANDED columns
scallop0322MainDataTable$DOLLAR<-NULL
scallop0322MainDataTable$POUNDS<-NULL
scallop0322MainDataTable$LANDED<-NULL

#### all = FALSE is used to keep only rows that match from the data frames
scallop0322MainDataTable <- left_join(scallop0322MainDataTable, Agg_DOL_POUN_LAND, by = "TRIPID")
...
scallop0322MainDataTable$LIVE_POUNDS <-POUNDS
scallop0322MainDataTable$MEAT_POUNDS <-POUNDS/8.33
scallop0322MainDataTable$POUNDS <-NULL
...
Average catch (meats, all observations). 
```{r}
zone_out <- 
  zone_summary(scallop0322MainDataTable, project = project,
           spat = scallop0322TenMNSQRSpatTable, 
...
Median catch (meats, all observations).
```{r}
zone_out <- 
  zone_summary(scallop0322MainDataTable, project = project,
           spat = scallop0322TenMNSQRSpatTable, 
...
zone_out <- 
  zone_summary(scallop0322MainDataTable, project = project,
           ...
           zone.spat = "MN10SQID",
           count = FALSE,
           var = "MEAT_POUNDS", fun = "median", group = "fleet",
           output = "tab_plot", na.rm = TRUE)
zone_out$plot
zone_out$table %>% 
  pretty_lab(cols = "MEAT_POUNDS") %>% 
...
```{r eval=params$LA_GCIFQ_only, echo=params$LA_GCIFQ_only}
zone_out <- 
  scallop0322MainDataTable %>% 
  filter(fleet == "GCIFQ") %>% 
...
Percent of total  catch (meat) all observations. 
```{r}
zone_out <- 
  zone_summary(scallop0322MainDataTable, project = project,
           spat = scallop0322TenMNSQRSpatTable, 
...
zone_out <- 
  ...
  zone_summary(project = project,
           ...
           breaks = c(1e3, 2.5e3, seq(5e3, 3e4, 5e3)),
           na.rm = TRUE, output = "tab_plot")
zone_out$plot
zone_out$table %>% 
