...
scallop0322MainDataTable <- 
  scallop0322MainDataTable %>%
  group_by(TRIPID) %>%
  mutate(Agg_DOLLAR = sum(DOLLAR), Agg_POUNDS = sum(POUNDS), Agg_LANDED = sum(LANDED), Agg_DOLLAR_ALL_SP_2020) %>% 
...
```{r eval=params$AA_DAS_only, echo=params$AA_DAS_only}
brks <- ifelse(!params$AA_DAS_only,
               c(100, 500, seq(1e3, 1e4, 3e3), seq(1.5e4, 4.5e4, 5e3)),
               c(100, 500, seq(1e3, 1e4, 2e3))) # TODO: update for AA_DAS
...
zone_out <- 
  ...
  zone_summary(scallop0322MainDataTable,
           ...
           # breaks = c(100, 500, seq(1e3, 1e4, 3e3), seq(1.5e4, 4.5e4, 5e3)),
           output = "tab_plot", na.rm = TRUE)
zone_out$plot
zone_out$table %>% 
  pretty_lab(cols = "MEAT_POUNDS") %>% 
...
zone_out <- 
  zone_summary(scallop0322MainDataTable, project = project,
           ...
           # breaks = brks, 
           breaks = seq(0, 2, .2),
           bin_colors = c("white", viridis::viridis(10)),
           output = "tab_plot", na.rm = TRUE)
zone_out$plot
zone_out$table %>% 
...
zone_out <- 
  ...
  zone_summary(project = project,
           ...
           var = "MEAT_POUNDS", fun = "percent",
           # breaks = seq(0, 2.4, .2),
           breaks = c(seq(0, .8, .2), seq(1, 2, .5)),
           bin_colors = c("white", viridis::viridis(7)),
           output = "tab_plot", na.rm = TRUE)
zone_out$plot
zone_out$table %>% 
  pretty_lab(cols = c("MEAT_POUNDS", "MEAT_POUNDS_perc"), type = "scientific") %>%
...
zone_out <- 
  zone_summary(scallop0322MainDataTable, project = project,
           ...
           breaks = c(2, 4, seq(6, 12, 1)),
           # breaks = c(1, seq(2, 14, 2)),
           output = "tab_plot", na.rm = TRUE)
zone_out$plot
zone_out$table %>% 
...
zone_out <- 
  ...
  zone_summary(project = project,
           ...
           count = FALSE,
           var = "CPUE", fun = "mean",
           # breaks = c(seq(250, 1e3, 250), seq(1.5e3, 3e3, 500)),
           na.rm = TRUE, output = "tab_plot")
zone_out$plot
zone_out$table %>% 
  pretty_lab(cols = "CPUE") %>%
...
zone_out <- 
  ...
  zone_summary(project = project,
           ...
           breaks = c(1e3, 2.5e3, seq(5e3, 3.5e4, 5e3)),
           na.rm = TRUE, output = "tab_plot")
zone_out$plot
zone_out$table %>% 
