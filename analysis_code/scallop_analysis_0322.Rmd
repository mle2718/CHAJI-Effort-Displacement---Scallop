...
```{r}
# helper functions
# these haven't been included in FishSET yet
source("~/FishSET_work/scripts/agg_helper2.R")
source("~/FishSET_work/scripts/zone_summary_fun.R")
source("~/scallop summary 0322/format_helpers.R")
...
# This code looks into data_intermediate and sets the vintage_string according to 
...
I needed to convert the geometry list column to separate lat/lon columns to be able
to save the data to the FishSET Database. The FishSET Databases are sqlite files, 
which can't handle spatial data.
```{r}
dat_filepath <- paste0("~/NE Scallops/data/updated/Final Product Lease - March ",
                       "2022/final_product_lease2022_03_08.Rds")

final_product_lease <- readRDS(dat_filepath)

# convert geometry column to lon lat 
final_product_lease <- 
  sfc_as_cols(final_product_lease, names = c("DDLON", "DDLAT")) %>% 
  sf::st_drop_geometry()
...
shape_filepath <- "~/Effort-Displacement---Scallop/data/external/shapefiles/"
lease_filepath <- paste0(shape_filepath, "All_Lease_Areas_Shapefile")
ten_filepath <- paste0(shape_filepath, 
                       "Ten Minute Squares Cut North and Greater Atlantic")
...
This check isn't perfect. For example, it doesn't tell us whether a column is 
mostly "empty", like the previous_port_lat/lon columns. 
```{r}
# previous_port_lat/lon contain nearly all NAs so I'll remove them
scallop0322MainDataTable <-
  scallop0322MainDataTable %>% 
  select(-previous_port_lat, -previous_port_lon)
```
<br><br>
...
fleet_tab <- data.frame(
  ...
  condition = c('`Plan Code` == "SES" & `Program Code` == "SAA"',
                ...
                '`Plan Code` == "SES" & `Program Code` == "SCG"'),
  fleet = c("Access Area", "Days at Sea", "General Cat")
)
...
scallop0322MainDataTable <- 
  assignment_column(scallop0322MainDataTable, project = project,
                    gridfile = scallop0322TenMNSQRSpatTable, 
                    lon.dat = "DDLON",
                    lat.dat = "DDLAT", 
                    cat = "MN10SQID",
                    closest.pt = FALSE,
...
scallop0322MainDataTable <- 
  assignment_column(scallop0322MainDataTable, project = project,
                    gridfile = scallop0322WindCloseSpatTable, 
                    lon.dat = "DDLON",
                    lat.dat = "DDLAT", 
                    cat = "NAME",
                    closest.pt = FALSE,
...
```{r}
agg_helper2(scallop0322MainDataTable, value = "in_closure", count = TRUE) %>% 
...
```{r}
agg_helper2(scallop0322MainDataTable, group = "fleet", 
...
```{r}
agg_helper2(scallop0322MainDataTable, value = "in_closure", 
...
zone_out <-
  zone_summary(scallop0322MainDataTable,
               ...
               zone.dat = "ZoneID",
               zone.spat = "MN10SQID",
               count = TRUE, var = "fleet", fun = "percent",
               output = "tab_plot",
               breaks = NULL, n.breaks = 10, na.rm = TRUE)
zone_out$plot
zone_out$table %>% 
...
Median live catch (General Category fleet).
```{r}
zone_out <- 
  scallop0322MainDataTable %>% 
...
```{r}
agg_helper2(scallop0322MainDataTable, 
            value = "closeID",
            group = "dup_trip",
            count = TRUE) %>% 
...
```{r}
agg_helper2(scallop0322MainDataTable, "POUNDS", 
...
```{r}
agg_helper2(scallop0322MainDataTable, "TRIP_LENGTH", 
            group = c("DB_LANDING_YEAR", "closeID"), 
            fun = "sum") %>% 
...
```{r}
agg_helper2(scallop0322MainDataTable, "CPUE", 
            group = c("DB_LANDING_YEAR", "closeID"), 
            fun = "mean") %>% 
  pivot_wider(names_from = DB_LANDING_YEAR, 
              values_from = CPUE,
...
```{r}
agg_helper2(scallop0322MainDataTable, "VPUE", 
            group = c("DB_LANDING_YEAR", "closeID"), 
            fun = "mean") %>% 
  pivot_wider(names_from = DB_LANDING_YEAR, 
              values_from = VPUE,
...
```{r}
agg_helper2(scallop0322MainDataTable, "NSUBTRIP", 
            group = c("DB_LANDING_YEAR", "closeID"), 
            count = TRUE) %>% 
  arrange(DB_LANDING_YEAR) %>% 
  pivot_wider(names_from = DB_LANDING_YEAR, 
