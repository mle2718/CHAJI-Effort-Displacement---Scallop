...
# Leave this behind for now, but delete soon.
...
By setting this code chunk to eval=TRUE, all of the statistics from this point forward 
are just for the Limited Access Fleet, which takes Access Area and Days-at-Sea trips. 
Currently set to `r params$AA_DAS_only`.
```{r retain_AA_DAS, eval=params$AA_DAS_only}
scallop0322MainDataTable <- 
  scallop0322MainDataTable %>% 
...
By setting this code chunk to eval=TRUE, all of the statistics from this point forward 
By setting this code chunk to eval=TRUE, all of the statistics from this point forward are for the Limited Access General Category IFQ (LAGC-IFQ) fleet. Currently set to
`r !params$AA_DAS_only`.
```{r retain_LA_GCIFQ, eval=!params$AA_DAS_only}
#```{r retain_LA_GCIFQ, eval=FALSE}
scallop0322MainDataTable <- 
  scallop0322MainDataTable %>% 
  filter(fleet == "GCIFQ")
```
...
### Bin Gears
```{r bin_gears, eval=params$BIN_GEARS}
...
scallop0322MainDataTable$OPERATING_PROFIT_2020 <- 
  (scallop0322MainDataTable$Agg_DOLLAR_ALL_SP_2020 - scallop0322MainDataTable$TRIP_COST_WINSOR_2020_DOL)
...
### CPUE
Here I creating a CPUE variable using TRIP_LENGTH and MEAT_POUNDS. I'm also filtering
...
```{r}
agg_helper(scallop0322MainDataTable, "MEAT_POUNDS", 
            group = c("DB_LANDING_YEAR", "closeID"), 
            fun = "sum") %>% 
  pivot_wider(names_from = DB_LANDING_YEAR, 
              values_from = MEAT_POUNDS,
              values_fill = 0) %>% 
  pretty_lab() %>% 
  pretty_tab_sb()
```
...
catch_out$table %>% 
  pivot_wider(names_from = GEARCODE, values_from = MEAT_POUNDS) %>% 
  pretty_lab() %>%
  pretty_tab_sb() 
