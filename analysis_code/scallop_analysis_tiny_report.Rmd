...
```{r}
scallop0322MainDataTable %>% 
  summarize(across(.cols = c("LANDED_KG","LANDED", "DOLLAR_2020" , "OPERATING_PROFIT_2020","TRIP_LENGTH"), 
                   .fns = list(min = min, median = median, mean = mean, max = max, StdDev = sd ,sum=sum, Q25 = ~quantile(.,0.25), Q50 = ~quantile(.,0.50), Q75 = ~quantile(.,0.75), Q95 = ~quantile(.,0.95),
...
catch_out$table %>% 
  pivot_wider(names_from = fleet, values_from = DOLLAR_2020) %>% 
  pretty_lab() %>%
  pretty_tab_sb() 
