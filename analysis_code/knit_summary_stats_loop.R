...
here::i_am("analysis_code/knit_summary_stats_loop")
...
input_shapefile_vals_to_loop<-c("wind_sf_nocable.RDS") # Different "closure areas". 
...
render_report = function(AA_DAS_only, subtrip_aggregate, input_shapefile) {
  rmarkdown::render(here("analysis_code","scallop_summary_stats.Rmd"), params = list(
      AA_DAS_only = AA_DAS_only,
      subtrip_aggregate = subtrip_aggregate,
      input_shapefile=input_shapefile
    ),
    output_file = here("results",paste0("scallop_summary_stats_AA_",AA_DAS_only,"_subtripaggregate_",subtrip_aggregate,"_shapefile_", gsub(".RDS", "",input_shapefile ), ".html")
  ))
}
################################################################################## 
