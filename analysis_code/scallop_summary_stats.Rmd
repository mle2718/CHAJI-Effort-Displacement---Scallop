...
scallop0322MainDataTable %>% 
  summarize(across(.cols = c("CREW", "TRIP_LENGTH"), 
                   .fns = list(min = min, median = median, mean = mean, sd=sd, max = max,
                               sum=sum, UniqueObs = n_distinct),
                   na.rm=TRUE, 
                   .names = "{.col}__{.fn}")) %>% 
  pivot_longer(cols = everything(), values_to = "value", names_to = "summary") %>% 
  pretty_lab() %>% 
...
```{r}
AA_data %>% 
  summarize(across(.cols = c("DOLLAR_2022_win", "DOLLAR_ALL_SP_win","LANDED_win", "VPUE_win","OPERATING_PROFIT_2022_win", "CREW", "TRIP_LENGTH"), 
                   .fns = list(min = min, median = median, mean = mean, sd=sd, max = max,
                               sum=sum, NAs = ~sum(is.na(.x)), UniqueObs = n_distinct, 
                               "No. 0's" = ~sum(.x == 0)), 
                   .names = "{.col}__{.fn}")) %>% 
