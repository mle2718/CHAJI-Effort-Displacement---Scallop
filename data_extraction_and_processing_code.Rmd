...
author: "Marina Chaji & Min-Yang Lee"
...
PKG <- c("here","leaflet", "tidyverse", "sf", "RODBC", "RODM", "dbplyr","raster","rgdal","readxl", "data.table", "tmaptools", "tmap", "dplyr", "epiDisplay")
for (p in PKG) {
...
for(i in START.YEAR:END.YEAR) {
  print(i)
  CURRENT.QUERY = paste("SELECT VTR.veslog",i,"t.TRIPID,tripcatg, operator, opernum, nsubtrip, crew,not_fished, port as VTR_PORTNUM  
...
# Verify we have a full set of PERMIT
stopifnot(sum(is.na(VTR_DMIS_merge$PERMIT))==0 ) 
...
#Subtrip totals with percentage breakdown
tab1(VTR_DMIS_AC$NSUBTRIP, sort.group = "decreasing", cum.percent = TRUE, graph = TRUE, xlab = "Number of Subtrips", ylab = "Number of Observations", main = "Count of Subtrips for Scallop Fishery")


#Average Trip Length by year and subtrip 
Average_Trip_Length_Scallop <- VTR_DMIS_AC %>% group_by(NSUBTRIP,YEAR) %>% summarise(tibble( mean = mean(TRIP_LENGTH)))
Average_Trip_Length_Scallop 

# Aggregated & average dollar, pounds, and landed values by subtrip
Dollar_POUND_LANDED_Scallops <- VTR_DMIS_AC %>% group_by(NSUBTRIP) %>% summarise(Aggregated_DOLLAR = sum(DOLLAR),Average_DOLLAR = mean(DOLLAR), Aggregated_POUNDS = sum(POUNDS), Average_POUNDS = mean(POUNDS), Aggregated_LANDED = sum(LANDED), Average_LANDED = mean(LANDED))
Dollar_POUND_LANDED_Scallops

```


```{r count_subtrips_limited_access, echo=TRUE}

#Table with all subtrips where LA is TRUE 
VTR_DMIS_AC_LA <- VTR_DMIS_AC %>% filter(LA == "TRUE")

#Subtrip totals with percentage breakdown - Limited Access 
tab1(VTR_DMIS_AC_LA$NSUBTRIP, sort.group = "decreasing", cum.percent = TRUE, graph = TRUE, xlab = "Number of Subtrips", ylab = "Number of Observations", main = "Count of Subtrips for Scallop Limited Access")


#Average Trip Length by year and subtrip - Limited Access 
Average_Trip_Length_LA <- VTR_DMIS_AC_LA %>% group_by(NSUBTRIP,YEAR) %>% summarise(tibble( mean = mean(TRIP_LENGTH)))
Average_Trip_Length_LA 


# Aggregated & average dollar, pounds, and landed values by subtrip - Limited Access 
Dollar_POUND_LANDED_Scallops_LA <- VTR_DMIS_AC_LA %>% group_by(NSUBTRIP) %>% summarise(Aggregated_DOLLAR = sum(DOLLAR),Average_DOLLAR = mean(DOLLAR), Aggregated_POUNDS = sum(POUNDS), Average_POUNDS = mean(POUNDS), Aggregated_LANDED = sum(LANDED), Average_LANDED = mean(LANDED))

Dollar_POUND_LANDED_Scallops_LA
