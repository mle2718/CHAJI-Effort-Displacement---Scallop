...
This code extracts and processes data. Our goal is to construct a dataset that can be used to estimate a location choice model for the Limited Access Scallop Fishery, using data from 2007-2019 (calendar years).  The main datasource is a frozen DMIS table.

# Dependencies

This code depends on:
1.  Two datasets from the offshoreWind package. These two datasets should be placed into the subfolder offshoreWind-master/data folder.  You can get these data from *fillin location*.
2. Trip cost estimtes. These can be found at  \\net\home6\swerner\\Drop_off\Trip_Cost_Estimation_2007_2017  
3. An ability to connect to NEFSC oracle databases (VTR and the Live DMIS tables at APSD.t_ssb_trip_current@garfo_nefsc)
...
# Project setup
Load packages,   Read in oracle username and password from "R_credentials.R".  
...
```{r oracle_connections, echo=FALSE}
source("R_credentials.R")
...
The frozen DMIS table from the offshoreWind project (APSD_DMIS_2) is the base dataset for the analysis.
...
Data in the VTR schema is needed.  We extract the 
     
1. TRIPID  - linking field 	
1. tripcatg  - Indicates whether this is a commercial trip, for hire trip, or another kind of trip 	
1. operator -   the name of the vessel operator
1. opernum - the idnumber for the vessel operator
1. permit -  the permit number of the vessel (vp_num) in the vps schema
1. nsubtrip -  the number of subtrips corresponding to the trip
1. crew 	- the number of crew 
1. not_fished -  Indicator for whether this was a fishing trip or not 	
...
#Add in cost data 
## This comes from the "Estimation of Commercial Fishing Trip Costs Using Sea Sampling Data" paper by Samantha Werner & Geret DePiper
```{r}
#Import Data
