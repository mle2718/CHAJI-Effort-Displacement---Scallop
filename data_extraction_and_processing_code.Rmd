...
PKG <- c("sp","leaflet", "tidyverse", "sf", "ggplot2", "RODM", "ROracle", "dbplyr", "dplyr", "raster","rgdal","readxl")
for (p in PKG) {
...
shapefile_path = file.path(net,"work5","socialsci","Geret_Rasters","Data","offshore_wind_package_data","shapefiles",
...
saveRDS(VTR_DMIS_AC_Agg, file = paste0("VTR_DMIS_AC_Agg",vintage_string,".rds"))
```
...
#################################################################################################
coordinate_table_input <- VTR_DMIS_AC_Agg
lat_column = "DDLAT"
lon_column = "DDLON"
#################################################################################################
...
# Finagle the input data (the LONs must be converted to negative, drop NAs.  )
...
#Pull out just the LON and LAT

xy <- coordinate_table[,c("LON","LAT")]
# convert data to a SpatialPointsDataFrame and do an spTransform
coordinate_table <- SpatialPointsDataFrame(coords = xy, data = coordinate_table, proj4string = crs)
# Next line probably unnecessary, because we are using data with 1 coordinate system and transforming it to the same coordinate system.
coordinate_table <- spTransform(coordinate_table, CRSobj = crs)



# Read in the land/water shapefile and coerce it to a SpatialPolygonsDataFrame, apply the crs projection, and make a column called "NAME" with the value "LAND"
shapefile_area = as(st_read(file.path(shapefile_path,"East_Cst_crop_2020_extended.shp")), "Spatial")
shapefile_area <- spTransform(shapefile_area, CRSobj = crs)
shapefile_area$NAME="LAND"
...
#Print a table of the number of observations on land
table(vtridx$NAME)
...
table(coordinate_table$Area)

 write.csv(coordinate_table, file=paste0("coordinate_table",vintage_string,".csv"))
```
